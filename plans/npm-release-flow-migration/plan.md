...
Adopt `vipentti/npm-release-flow@0.1.0` as the sole release owner. Ship the first kit release `0.6.0` and delete the bespoke release path so only the thin caller remains. Outcome is a single releaser with no dual paths.
...
- `scripts/` release helpers (`scripts/release.mjs`, `detect-release-merge.mjs`, `verify-release-tag.mjs`, `scripts/verify-release-tag.d.mts` and related tests/docs) and `knip.json` entry for `scripts/verify-release-tag.d.mts` plus npm scripts `release:prepare` / `release:tag` already removed in this PR via atomic ownership transfer at ddca69d/3cb11b0 (GitHub reruns preserve the original commit SHA and its workflow/source, so no coexistence window is needed); `scripts/assert-changelog-release-ready.mjs` is PRESERVED for its ordinary no-flag CI lint duty (cardinality, valid dates, non-empty notes, link refs) and shrunk to that mode only — release-specific flags/branches and release-mode tests deleted, release validation owned by `npm-release-flow`.
...
7. pin advances only when upgrading the kit version, atomically with exact `devDependency` in one ordinary PR; no advance after consumer `0.6.0`.
...
- T5: first kit release `0.6.0` via `npx npm-release-flow prepare --version 0.6.0 --execute` (dry-run is `npx npm-release-flow prepare --version 0.6.0`). Post-prepare: update the generated release PR body to `.github/pull_request_template.md` while preserving the exact `Kit: @vipentti/npm-release-flow@0.1.0` line required for kit detection. Repository-approved exception: the exact-three-file release PR (`CHANGELOG.md`, `package.json`, `package-lock.json`) MUST NOT carry `plans/*` task-state changes even when the PR would otherwise finish a planlet task per `AGENTS.md`; T5's `task check` is recorded in the immediate post-release ordinary push (or a follow-up branch) so the release allowlist stays exact and `AGENTS.md` bookkeeping is satisfied without conflict.
- T7: post-`0.6.0` repository cleanup — removal of legacy `RELEASE_*` environment secrets/variables and final migration bookkeeping. Release-ownership source deletion (scripts, knip entries, workflow-coupled tests, `RELEASING.md`/`AGENTS.md` rewrites, `assert-changelog-release-ready.mjs` shrunk to no-flag lint) already landed in this PR at ddca69d/3cb11b0/this revision; `RELEASING.md` states no coexistence window is needed.

T7 live-contract decisions: release-ownership source deletion already performed in this PR (deleted `scripts/release.mjs`/`detect-release-merge.mjs`/`verify-release-tag.mjs`/`scripts/verify-release-tag.d.mts`, `release:prepare`/`release:tag`, `knip.json` ignores, workflow-coupled tests, shrunk `scripts/assert-changelog-release-ready.mjs` to ordinary no-flag CI lint and removed `--release-date`/`--verify-release`/`--print-date` branches plus `countFlags`/`assertNotPast`/`packageLockMismatch`/`packageIdentityMismatch`/`updateChangelogLinkReferences` release exports and corresponding `tests/integration/changelog.test.ts` release-mode coverage; updated `AGENTS.md` `Rejected simplifications` to reflect helper as ordinary lint only and `RELEASING.md` to kit flow; `ci.yml` step `node scripts/assert-changelog-release-ready.mjs` stays no-flag). Remaining T7 work that must wait until after `0.6.0` ships: delete old `RELEASE_*` environment secrets and variables; verify zero live references to deleted release helpers and the declaration path remain outside `plans/` (active planlet still mentions deleted helpers/legacy names) using anchored patterns `\bRELEASE_GPG_`, `\bRELEASE_APP_`, `\bRELEASE_GIT_` and `scripts/verify-release-tag\.d\.mts` that do not match `NPM_RELEASE_FLOW_`.

Not a task: kit version upgrades bump caller pin with the exact `devDependency` in one ordinary PR; documented here and in `RELEASING.md`. Ordering: T1 before T4; T2 before T5; T7 waits on T5; legacy `RELEASE_*` secrets/variables persist until T7 so reruns stay green (source already cleaned in this PR, no file-coexistence needed — reruns use their commit's workflow/source).
...
- Kit release `0.6.0` is signed, provenance-published and has a GitHub Release.
