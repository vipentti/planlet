...
inside a deterministic CLI, and several hygiene smells (prettier drift across a
mix of generated, archived, and maintained files, leaked integration-test temp
fixtures, bare non-null map-lookup assertions, an unguarded `realpathSync`, and an
assertion against stringified `"undefined"`). Each item is small; together they reduce maintenance
...
- **Hygiene (T5):** classify the drifted files by ownership instead of a blanket
  ignore: narrowly ignore the two generated manifests and the `plans/completed/**`
  archive pattern (documented policy), and format the two maintained source
...
1. **Prettier drift** — the nine files currently reported by `prettier --check`
   split by ownership:

   - **Generated manifests (ignore):** `.agents/skills/.planlet-manifest.json` and
     `.claude/skills/.planlet-manifest.json` are generated by `planlet update`;
     their canonical generator owns their formatting, so they stay narrowly
     ignored in `.prettierignore`, not reformatted by hand.
   - **Historical archived planlets (ignore as archive policy):**
     ...
     `plans/completed/2026-08-03-launch-readiness/tasks.md`, and
     `plans/completed/2026-08-04-release-cut-utility/plan.md`. Completed planlets
     are preserved, immutable audit records (the repo refuses to silently overwrite
     them and keeps them structurally inspectable), so ignore them with the single
     documented archive-policy pattern `plans/completed/**` rather than five
     one-off entries. This is a stated policy, not a drift-silencing hack.
   - **Maintained source artifacts (format, do not ignore):**
     `planlet_design.md` and `tests/fixtures/skills/scenarios.json` are formatted
     with Prettier and added — as the exact `planlet_design.md` path and the exact
     `tests/fixtures/skills/scenarios.json` path (not a broad JSON glob) — to both
...
- `npm run format:check` is the authoritative formatting gate; `npm run format`
  and `npm run format:check` cover the same project-owned inputs (now including
  `planlet_design.md` and `tests/fixtures/skills/scenarios.json`).
  `prettier --check .` is a one-off informational scan only and is not a required
  acceptance criterion. `.prettierignore` carries only policy-based patterns
  (generated manifests and the `plans/completed/**` archive pattern), not entries
  added merely to silence current drift.
