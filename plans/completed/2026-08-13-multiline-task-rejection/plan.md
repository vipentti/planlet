...
The fix makes the logical task model insensitive to harmless soft wrapping. A recognized task keeps one physical checkbox line as the marker, but its description may soft-wrap across following indented lines belonging to the same list item, including nested valid Markdown lists (unordered `- ` `* ` `+ ` and ordered `1. `) and indented paragraph continuations (for example 6-space wrapped prose). `parseTasks` consumes those following lines and normalizes their whitespace into the parsed description. Blank lines or new Markdown block constructs (heading `#{1,6} `, blockquote `> `) end the description. Nested checkbox syntax stays invalid via existing parser precedence, and `task check`/`uncheck` stays byte-preserving on the checkbox line.
...
- Consume and normalize indented continuation lines in `src/core/plan/task-parser.ts:parseTasks` (uses `TASK_LINE_PATTERN` 9-11, `parseTaskLine` 27-45, `TASK_LIKE_LINE_PATTERN` 18-19), including nested valid Markdown lists (unordered `- ` `* ` `+ ` and ordered `1. `) and indented paragraph continuations (for example 6-space wrapped prose). Stop consuming at blank lines or new Markdown block constructs (heading `#{1,6} `, blockquote `> `) so headings and blockquotes remain separate Markdown. Keep `TASK_LIKE_LINE_PATTERN` precedence: an indented line that looks like a malformed task such as `  - [ ] T2 Nested` is still rejected as `invalid_plan` with details `{ line, content }` and no `taskId` before consumption applies.
...
- Keep single-line tasks valid and ordinary Markdown outside tasks unaffected. Completed archives remain valid without migration, since consumed lines now include nested valid lists; for example acceptance sub-bullets such as those in `plans/completed/2026-08-06-copilot-detect-signals/tasks.md` that start with `  - ` are consumed into the preceding task description with normalized whitespace.
...
3. Implement consumption in `parseTasks`:
   ...
   - When `parseTaskLine(line)` is non-null, capture its base description and task id. Then peek forward while the next line exists:
     ...
     - Else if `nextLine` is blank (`/^[ \t]*$/`) or starts a new Markdown block construct after optional indent (`/^[ \t]*(?:#{1,6}\s|>\s)/`) then break, do not consume. Nested valid Markdown lists (`- ` `* ` `+ `, `\d+\. `) are not treated as block boundaries and are consumed when indented as `  - ` or `  1. `.
...
5. Update documentation: `planlet_design.md` 10.4 states one physical checkbox line per task, description may soft-wrap across following indented lines including nested valid Markdown lists and indented paragraph continuations that are consumed and normalized, blank lines or new block constructs (heading, blockquote) end the description, nested checkbox stays invalid, extended detail still belongs in `plan.md` when it is more than a wrapped sentence. `skills/planlet-plan/assets/tasks-template.md` shows single-line examples and notes the same soft-wrap rule. `CHANGELOG.md` `[Unreleased]` records the user-visible parsing change.
...
6. Add focused tests:
   ...
   - Unit and CLI: wrapped task still validates (exit 0), single-line tasks remain valid, ordinary Markdown (prose before first task, blank-line-separated paragraph, heading, next task) unaffected, indented paragraph continuation and nested valid lists (`  - item`, `  1. Step`) after a task are consumed.
...
- `parseTasks` consumes following indented lines belonging to the same list item into the task description, including nested valid Markdown lists and indented paragraph continuations, normalizing whitespace with single spaces. A blank line, heading, or blockquote ends the description.
...
- A nested valid Markdown list such as `  - Acceptance` or `  1. Step 1` after a task is consumed into the preceding task description with normalized whitespace (for example `  1. Step 1` becomes `1. Step 1` joined with a space), so existing completed archives such as `plans/completed/2026-08-06-copilot-detect-signals/tasks.md` remain valid and their descriptions reflect the normalized list content.
...
- New block boundary detection must be narrow. Only blank lines and lines that start a new block construct (heading, blockquote) after optional indent end consumption, so plain paragraph wraps and nested valid Markdown lists are consumed but headings and blockquotes are not. Keep prose, regex, and tests aligned.
