...
- New dependency-free `scripts/verify-release-tag.mjs` for the break-glass
  `scripts/release.mjs` path. The protected workflow deliberately duplicates
  minimal Git verification inline because it executes no repository-owned
  scripts.
- Minimal `scripts/release.mjs` adjustment: delegate local-tag validation to
  the break-glass verifier; `prepare`, dry-run defaults, `tag` break-glass flow, and
...
- Focused subprocess tests for detection and the break-glass tag verifier; workflow
...
verify locally with the inline Git verifier; push only that ref with a
...
write only) through temporary `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_*`/
`GIT_CONFIG_VALUE_*` environment variables carrying a basic-auth extraheader
(never in git arguments, URLs, or logs); the token is generated only when the
remote tag is absent.
...
and `git verify-tag` success via the inline Git verifier; mismatched state fails
...
this helper (message `v<version>`, target `HEAD`). The protected workflow
deliberately duplicates minimal Git verification inline because it executes no
repository-owned scripts. All other `tag` guards, dry-run behavior, and
push/remote verification stay in `release.mjs`. Fixture
...
- Focused subprocess tests under `tests/integration/` for the detection script
  and the break-glass tag verifier, using temp git repos with bare local remotes and
