...
- `RELEASING.md` rewrite of the normal release procedure, required environment
  secrets/variables, token and tag-ruleset setup, rerun behavior, recovery,
...
The detection job never references `environment:`, never reads
`RELEASE_*` or `RELEASE_PUSH_TOKEN`, and cannot start the protected job.
...
verify locally with the shared tag verifier; push only that ref with
`RELEASE_PUSH_TOKEN` through a `git -c http.extraheader=...` basic-auth header
(never in the URL or logs); then re-verify the remote ref points at the exact
tag object. Non-force push only; a ruleset rejection fails the job with git's
diagnostic.
...
`RELEASE_PUSH_TOKEN`) and variables (`RELEASE_GPG_FINGERPRINT`,
`RELEASE_GIT_NAME`, `RELEASE_GIT_EMAIL`); dedicated release-only GPG key whose
email is verified on the GitHub account holding the public key; fine-grained
token scoped to `vipentti/planlet` with repository Contents read/write and an
actor allowed to bypass the existing `v*` tag-creation restriction; updates,
force changes, and deletions remain prohibited by the ruleset; a dedicated
GitHub App on the ruleset bypass list is the preferred future replacement for a
maintainer PAT; rerun behavior; `release:tag` as break-glass recovery (no
longer the normal happy path); key rotation at a high level; and that merging
another release PR before the previous release completes is unsupported.
...
- Only the release job references `environment: release`; only its steps access
  `RELEASE_*` secrets and the push token; workflow permissions stay minimal
...
- GPG tag creation meets every signing requirement: isolated temp home,
  ...
  single non-force tag push via the push token, GitHub verified-signature
...
- External setup is a hard dependency: token actor must be allowed to create
  `v*` tags under the existing ruleset; until a dedicated GitHub App bypass is
  configured, a maintainer PAT is the only actor.
