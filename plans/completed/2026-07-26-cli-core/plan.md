...
Write each domain module's and command's tests alongside its implementation rather than as a deferred pass, so a task is not complete until its own behavior is verified. Use unit tests for validation, parsing, state derivation, archive-name handling, TOON rendering, and error mapping. Use disposable repositories for discovery, initialization, multiple active plans, task updates, completion records and movement, incomplete overrides, and stdout/stderr behavior. Cover cross-cutting safety concerns that span several commands — collisions, malformed data, path traversal, and symlink escape — as their own dedicated pass once the affected commands exist. Write and run all of these tests as TypeScript files executed directly via `tsx`'s `node:test` integration; prefer invoking command handlers in-process (see Command and output model) over spawning the compiled executable, reserving compiled-executable fixtures for end-to-end argv, exit-code, and stdout/stderr framing checks. Test supported Node versions in CI when CI is introduced.
