...
7. Run `task check` before committing the implementation it records; a checkbox flip must never trail its implementation across a commit boundary. When committing completed work, include the corresponding staged `tasks.md` changes in that commit; multiple completed tasks may be intentionally batched into one commit. `task check` writes `tasks.md` and stages it, so the flip is swept into the commit that follows. If the implementation commit already exists and is unpushed, `git commit --amend --no-edit` sweeps the staged flip into it. Before switching branches or pushing, confirm with `git status --short` that no planlet-file changes remain uncommitted, and review `git diff --cached` so the commit carries only intended content.
