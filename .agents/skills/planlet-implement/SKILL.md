...
7. Run `task check` before any commit that includes the implementation it records. A checkbox flip must never trail its implementation across a commit boundary. Keep corresponding staged `tasks.md` changes with that implementation when it is eventually committed. Multiple completed tasks may share one commit, and when the user intends to complete the planlet in the same workflow, implementation, checkbox updates, and subsequent completion/archive changes may all be committed together. If implementation was already committed without its checkbox update, repair history only when that commit is unpushed and safe to rewrite; otherwise preserve existing history and ensure the Planlet update is committed before pushing. Before pushing or switching branches, confirm no Planlet state changes are left outside the commits that contain the repository state they describe. Review `git diff --cached` before committing so only intended content is included.
...
Run final `planlet --root "<repository-root>" tasks <slug> --remaining` and `planlet --root "<repository-root>" status <slug>`. Before reporting, inspect `git status --short`. Do not leave Planlet changes uncommitted if this workflow is ending, pushing, or switching branches. If the user is immediately continuing into the separate completion workflow, staged Planlet changes may remain pending so implementation and completion can be committed atomically. Report logical slug, outcomes, task IDs checked during this run, exact verification and results, deviations or blockers, remaining task IDs, and canonical state. State whether state is `ready_to_complete`.
