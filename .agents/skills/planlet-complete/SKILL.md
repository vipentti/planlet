...
Report any optional `## Verification Evidence` section in `tasks.md` as inspected evidence. Treat it as opaque prose: do not parse its semantics, rerun its checks, create missing proof, or accept it in place of a checked task. If the plan's strategy names a mandatory external gate whose evidence is absent, report that and leave the affected task unchecked.
