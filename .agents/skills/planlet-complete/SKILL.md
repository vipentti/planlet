...
Include the archive move and completion record in the commit that represents completion. If verified implementation and its Planlet task updates are still intentionally staged from the immediately preceding implementation workflow, they may be committed together with completion as one atomic commit. Do not require a separate completion commit. Before pushing, switching branches, or ending with no further commit expected, confirm `git status --short` shows no uncommitted Planlet changes. Report logical slug, recorded UTC timestamp, mode, remaining task IDs for override, final archive path, whether an optional evidence section was present, and post-completion validation result. If operation stopped, report exact source state and blocking code.
