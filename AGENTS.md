...
- **`--release-date` removed from `scripts/assert-changelog-release-ready.mjs`** — the helper is ordinary structural lint only (`node scripts/assert-changelog-release-ready.mjs` with no flags: exactly one `[Unreleased]`, at most one dated `[pkg.version]` with valid calendar date, non-empty notes, and link refs). Release cut date is `todayUtc()` inside `vipentti/npm-release-flow`; historical `--verify-release` / `--release-date` / `--print-date` and the `countFlags` / `assertNotPast` release branches were deleted with that ownership transfer. Keeping speculative release flags after the sole-owner migration would reintroduce a second release path. `RELEASING.md` documents the kit flow and `ci.yml` invokes the helper with no flags.
