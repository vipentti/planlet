...
- **Lock ownership token** (`src/core/planlet-lock.ts`). Not PID-reuse insurance — a dead process never releases. It makes the recovery our own error text invites survivable: a user who removes a lock that turned out to be live lets a second holder in, and the first holder's later release would delete that holder's lock mid-write.
- **`isProcessAlive` treating non-`ESRCH` as alive.** One comparison, not a table of handled codes. Rethrowing the rest crashes the CLI on the `EPERM` a shared `/tmp` returns for another user's process.
...
- **Lock dependency injection** (`write`, `remove`, `isProcessAlive`, `pid`). The seams for the acquire-failure and release-failure tests. Removing them removes those tests.
