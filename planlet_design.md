...
Task-file writes should be atomic. Mutating CLI operations that rewrite `tasks.md` (`task check`, `task uncheck`, and `complete`) take an exclusive per-planlet lock. Locks live outside the repository, in a per-owner, per-repository-root directory under the OS temp directory, so a transient holder file never appears in `git status` or an editor tree. Each acquisition writes an ownership token into the holder file and returns that token with the lock path. Competing writers fail with `write_conflict` rather than applying a stale read-modify-write. Stale locks whose recorded holder PID is dead are reclaimed by removing the holder file and retrying acquisition, and only a definite "no such process" counts as dead. Reclaim is therefore not race-free: two processes that observe the same dead holder can both remove it and both acquire. No file-based scheme closes that window, because POSIX offers no atomic compare-and-delete; bind `flock(2)`/`LockFileEx` if it ever must be closed. Release deletes the lock only when the caller's ownership token still matches the holder. Live holders are never stolen. Editors and git can still rewrite `tasks.md` outside the CLI lock; treat that as ordinary version-control conflict resolution.
