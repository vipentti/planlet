...
path-scoped `git add -A` when the moved planlet was already tracked or staged
(source deletion and destination staged together), plain `git add` when it was
...
scaffold stubs and does not stage them. The CLI never commits. Include staged Planlet state in the same commit as the repository state it records; task and completion changes may be batched into one commit.
...
- [Changelog and releasing](https://github.com/vipentti/planlet/blob/main/RELEASING.md)
