...
7. Commit each task's checkbox flip in the same commit as the task's implementation, before starting the next task or pushing. `task check` writes `tasks.md` and stages it — intentional, so the flip is swept into the commit that follows. Confirm with `git status --short` that the commit holds the implementation files and `tasks.md` together. Never push with planlet-file changes uncommitted: a flip left for later lands in the wrong commit or never reaches the remote.
