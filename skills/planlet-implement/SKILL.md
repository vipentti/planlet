...
7. Run `task check` before committing the implementation it records. Planlet state must never trail the repository state it describes across a commit, push, or branch boundary. Include corresponding staged Planlet changes whenever that repository state is committed. Multiple tasks, and subsequent completion/archive changes, may share one commit. If implementation was already committed without its Planlet update, rewrite that commit only when unpushed and safe; otherwise commit the missing Planlet state before pushing. Before committing, pushing, or switching branches, inspect the relevant Git state and ensure only intended changes cross the boundary.
...
Run final `planlet --root "<repository-root>" tasks <slug> --remaining` and `planlet --root "<repository-root>" status <slug>`. Before reporting, inspect `git status --short`. Do not leave Planlet changes uncommitted when ending, pushing, or switching branches. They may remain staged when continuing directly into Planlet completion so implementation and completion can share one commit. Report logical slug, outcomes, task IDs checked during this run, exact verification and results, deviations or blockers, remaining task IDs, and canonical state. State whether state is `ready_to_complete`.
