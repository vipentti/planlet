...
7. Commit each task's checkbox flip in the same commit as the task's implementation — the flip must never cross a commit boundary — before starting the next task, switching branches, or pushing. `task check` writes `tasks.md` and stages it — intentional, so the flip is swept into the commit that follows. If the implementation commit already exists and is unpushed, `git commit --amend --no-edit` sweeps the staged flip into it. Confirm with `git status --short` that the commit holds the implementation files and `tasks.md` together, and review `git diff --cached` so the commit carries only intended content. Never push with planlet-file changes uncommitted: a flip left for later lands in the wrong commit or never reaches the remote.
