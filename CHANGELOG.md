...
- Add read-only `planlet check-completion --base <git-ref>` gate for reporting changed active planlets that are ready to complete, and run it in Planlet's Linux Node 22 CI job against each pull request's base SHA.
